...
def upload_cooling_rate():
    if request.method == 'POST':
        ...
        if file:
            ...
            flash("File uploaded and read successfully", "success")
...
def train_model():
    ...
    if not data_dict:
        flash("No data found in session. Please upload the data file first.", "error")
        return redirect(url_for('upload_cooling_rate'))

    data = pd.DataFrame.from_dict(data_dict)
    required_columns = [
        'Austempering Temperature (°C)', 'Austempering Time (minutes)', 
        'Initial Temperature (°C)', 'Carbon (%)', 'Silicon (%)', 
        'Manganese (%)', 'Sulfur (%)', 'Phosphorus (%)', 
        'Chromium (%)', 'Nickel (%)', 'Molybdenum (%)', 
        'Cooling Rate (°C/min)', 'Tensile Strength (MPa)', 
        'Hardness (HB)', 'Elongation (%)'
    ]
    if not all(col in data.columns for col in required_columns):
        flash("Missing required columns in the uploaded data", "error")
        return redirect(url_for('upload_cooling_rate'))
    ...
    return render_template('train_model.html', mse_values=mse_values, best_model_name=best_model_name, best_mse=best_mse, model_path=model_path)
...
def download_model():
    ...
    if os.path.exists(model_path):
        ...
        flash("No model available for download. Please train the model first.", "error")
        return redirect(url_for('train_model'))
...
def predict():
    if request.method == 'POST':
        ...
        if os.path.exists(model_path):
            ...
            print(f"Predicted cooling rate: {predicted_cooling_rate}")
...
def upload_composition():
    if request.method == 'POST':
        ...
        if file:
            ...
            flash("Composition data uploaded and read successfully", "success")
