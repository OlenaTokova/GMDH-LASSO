...
def upload_cooling_rate():
    if request.method == 'POST':
        ...
        if file:
            ...
            print(f"File uploaded and read successfully: {file_path}")
...
def train_model():
    ...
    if data_dict:
        ...
        if not all(col in data.columns for col in required_columns):
            print("Missing required columns in the uploaded data")
            return "Missing required columns in the uploaded data", 400
        ...
        print(f"Model trained successfully. Best model: {best_model_name} with MSE: {best_mse}")
